...
- **会话自动命名**：新建会话完成第一轮对话（用户提问 + AI 回复）后，自动用当前模型总结一个 20 字以内的标题并更新左侧列表；已手动重命名或已生成过标题的会话不会被覆盖，生成失败则保持原标题不变（静默，不报错）。
- **技能调用折叠展示**：用户消息里调用的技能（`/skill:XXX`）不再展开整段 SKILL.md 全文，而是折叠成一个 `skill:XXX` 链接，点击在右侧预览抽屉打开完整 SKILL.md，也可就地展开查看内容；技能全文仍照常发给 agent，不影响功能。
