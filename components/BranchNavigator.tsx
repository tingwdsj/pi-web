...
interface Props {
  ...
  compact?: boolean;
  /** When true, the trigger button is greyed out and non-interactive (e.g. while streaming) */
  disabled?: boolean;
  /** When true, render ONLY the tree panel content (no trigger button / header),
   *  for embedding inside an externally-controlled drawer (e.g. the top panel). */
  panelOnly?: boolean;
}
...
export function BranchNavigator({ tree, activeLeafId, onLeafChange, inline, containerRef, open: openProp, onToggle, hasSession, compact, disabled, panelOnly }: Props) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setDropdownPos({ top: rect.bottom, left: rect.left, width: Math.max(rect.width, 240) });
    };
    update();
    ...
    return (
      <div style={{ background: "var(--bg-panel)", borderBottom: "1px solid var(--border)" }}>
        {hasContent && firstNode ? (
          <div style={{ padding: "6px 12px 10px 12px", maxHeight: "min(420px, 60vh)", overflowY: "auto" }}>
  ...
  if (inline) {
    return (
      <div style={{ height: 32, display: "flex", alignItems: "stretch" }}>
        <button
          ref={btnRef}
          onClick={() => { if (disabled) return; onToggle ? onToggle() : setOpenInternal((v) => !v); }}
          disabled={disabled}
          style={{
            display: "flex",
            alignItems: "center",
            ...
            gap: 5,
            height: 32,
            padding: compact ? "0 6px" : "8px 12px",
            ...
            whiteSpace: "nowrap",
            transition: "color 0.1s, background 0.12s, opacity 0.12s",
          }}
          onMouseEnter={(e) => { if (!disabled) { e.currentTarget.style.background = "var(--bg-hover)"; e.currentTarget.style.color = "var(--text)"; } }}
          onMouseLeave={(e) => { e.currentTarget.style.background = open ? "var(--bg-hover)" : "none"; e.currentTarget.style.color = open ? "var(--text)" : "var(--text-muted)"; }}
          title="分支"
          aria-label="分支"
          aria-pressed={open}
        >
